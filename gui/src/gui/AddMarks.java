...
class AddMarks {
    ...
    boolean checkValid() {
        try {
            for (Student student : currentSubject.studentsEnrolled) {
                ArrayList<Integer> atten = student.marks.get(currentSubject);
    ...
    class SaveButtonListener implements ActionListener {
        public void actionPerformed(ActionEvent ev) {
            ...
            if (isValid == true) {
                for (Student student : currentSubject.studentsEnrolled) {
                    ArrayList<Integer> marks = student.marks.get(currentSubject);
                    int rec = Integer.parseInt(map.get(student).getText());
                    marks.add(rec);
                    student.marks.put(currentSubject, marks);
                }
                frame.dispose();
            } else {
